...
class ChromaDBMemoryProvider(MemoryProvider):
    ...
    def _embed(self, texts: List[str]) -> Optional[List[List[float]]]:
        """Embed texts using ForgeEmbeddingFunction.

        Returns None if no EF is available — caller should fall back
        to passing documents= (ChromaDB default embedding).
        """
        if self._ef is None:
            return None
        try:
    ...
    def _upsert(self, collection, ids, documents, metadatas=None):
        """Upsert with explicit embeddings when available."""
    ...
    def _query(self, collection, query_text: str, n_results: int = 10,
        ...
        """Query with explicit embeddings when available."""
        if include is None:
    ...
    def search_team_ops(
        ...
        try:
            ...
            if agent_name:
                kwargs["where"] = {"agent_name": agent_name}
            results = self._query(collection, query, n_results=n_results, where=where_filter)
            return self._format_results(results)
        except Exception as e:
