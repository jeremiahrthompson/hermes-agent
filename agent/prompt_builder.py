...
KANBAN_GUIDANCE = (
    ...
    "the canonical work package for scope, routing, predecessor handoffs, "
    "and retry diagnostics. It is not a substitute for source artifacts when "
    "the task is about code, data, citations, tests, reviews, or claims.\n"
    ...
    "## Authority boundary\n"
    "\n"
    "Kanban governs **execution order, status, coordination, and handoff** — "
    "what runs next, who owns it, and what predecessors handed across. It is "
    "**not** a claim, evidence, or proof authority. Marking a task `done` means "
    "a worker handed off; it does not certify the underlying work is correct, "
    "reproducible, or verified.\n"
    "\n"
    "For research, regulated, safety-critical, financial, or security work, "
    "the authoritative record is the **source artifacts**: repositories, tests, "
    "datasets, citations, signed commits, review records, and immutable run "
    "receipts. `summary` and `metadata` must point at those artifacts; parent "
    "handoff prose is a pointer to verify, not proof to promote.\n"
    "\n"
    ...
    "- Do not treat a parent task's `done` status, summary, metadata, "
    "comments, or run logs as proof its claims are correct; for high-stakes "
    "work, verify against the source artifacts they reference.\n"
    "- Do not use `kanban_complete` as the only place a research, safety, "
    "financial, or security finding exists; write the finding into the source "
    "artifact and have the board handoff point at it.\n"
