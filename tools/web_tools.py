...
def _get_backend() -> str:
    ...
    valid_backends = {"parallel", "firecrawl", "tavily", "exa", "searxng", "brave-free", "ddgs", "direct"}
    ...
    return "direct"  # keyless fallback when no configured backend is usable
...
def web_search_tool(query: str, limit: int = 5) -> str:
    ...
    try:
        ...
        # Dispatch through the web search registry. Providers live as plugins;
        # the dispatcher is just a registry lookup + delegation.
        ...
        if provider is None:
            raise RuntimeError(
                "No web search provider configured. Set FIRECRAWL_API_KEY, "
                "TAVILY_API_KEY, EXA_API_KEY, PARALLEL_API_KEY, "
                "BRAVE_SEARCH_API_KEY, or SEARXNG_URL; or run `hermes tools` "
                "to set one up."
            )
        else:
            logger.info(
    ...
    try:
        ...
        if not safe_urls:
            ...
            if backend == "direct":
                ...
                # All providers live as plugins. The dispatcher is a registry
                # lookup + delegation. Some providers' extract() is async
                # (parallel, firecrawl), others sync (exa) — detect and await
                # or run in a worker thread as appropriate.
                from agent.web_search_registry import (
    ...
    try:
        ...
        if backend == "direct":
            ...
            if use_llm_processing and auxiliary_available:
                ...
                for result_item in processed_results:
                    if isinstance(result_item, BaseException):
                        logger.warning("Direct crawl processing task failed: %s", result_item)
                        continue
                    result, metrics, status = result_item
                    if status == "processed":
                        debug_call_data["compression_metrics"].append(metrics)
        ...
        return json.dumps(
            {
                "success": False,
                "error": (
                    "web_crawl requires Firecrawl or another crawl backend. "
...
def check_web_api_key() -> bool:
    ...
    if configured in {"exa", "parallel", "firecrawl", "tavily", "searxng", "brave-free", "ddgs", "direct"}:
        return _is_backend_available(configured) or _is_backend_available("direct")
    return any(
        _is_backend_available(backend)
        for backend in ("exa", "parallel", "firecrawl", "tavily", "searxng", "brave-free", "ddgs", "direct")
    )
...
if __name__ == "__main__":
    ...
    if web_available:
        ...
        if backend == "exa":
            ...
            print("   Using keyless direct fallback (DuckDuckGo HTML + Jina reader)")
