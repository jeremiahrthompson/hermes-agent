...
def unblock_task(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    with write_txn(conn):
        ...
        if not stale:
            return False
        unfinished_reconciler_parent = conn.execute(
            "SELECT p.id FROM task_links l "
            "JOIN tasks p ON p.id = l.parent_id "
            "WHERE l.child_id = ? AND p.status != 'done' AND p.metadata IS NOT NULL",
            (task_id,),
        ).fetchall()
        for parent in unfinished_reconciler_parent:
            if _guardrail_role(_task_meta(conn, parent["id"])) == "reconciler":
                return False
