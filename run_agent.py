...
# ── Memory prompt-source policy (core, provider-agnostic) ─────────────
# Enum for memory.prompt_source. See hermes_cli/config.py for the per-value
# semantics; the core policy lives in AIAgent._build_system_prompt_parts.
_VALID_MEMORY_PROMPT_SOURCES = frozenset(
    {"legacy", "shadow", "provider_with_legacy_fallback", "provider"}
)

# Visible marker injected when prompt_source == "provider" and neither live
# generation nor cache produced a usable block. Kept short and operator-readable
# so it shows up clearly in the rendered system prompt.
_MEMORY_PROVIDER_UNAVAILABLE_MARKER = (
    "# Memory Provider Unavailable — no profile loaded this session"
)


def _provider_block_has_generated_profile(block: str) -> bool:
    """Return True for a provider-owned generated profile wrapper.

    The wrapper must start at a line boundary and advertise
    ``source="chromadb"`` (or single-quoted equivalent). This avoids letting
    status/team-knowledge text that merely contains ``<memory-profile`` decide
    legacy-memory suppression.
    """
    if not block:
        return False
    return bool(re.search(
        "(?im)^<memory-profile\\b(?=[^>]*\\bsource=[\\\"']chromadb[\\\"'])",
        block,
    ))


def _provider_block_is_degraded(block: str) -> bool:
    """Return True when a rendered provider block advertises degraded mode.

    Core remains provider-agnostic: we look only for the
    ``degraded="true"`` marker that the generated-profile contract
    requires on the ``<memory-profile>`` opening tag. Anything else is
    treated as non-degraded.
    """
    if not block:
        return False
    needle = block.lower()
    return 'degraded="true"' in needle or "degraded='true'" in needle
...
class AIAgent:
    ...
    def __init__(
        ...
        # See hermes_cli/config.py "memory.prompt_source" for the enum.
        # Core does not query ChromaDB or any specific backend; it only
        # decides whether to suppress the legacy MEMORY/USER markdown
        # blocks based on the policy and the rendered provider block.
        ...
        if _prompt_source not in _VALID_MEMORY_PROMPT_SOURCES:
            logger.warning(
                "Unknown memory.prompt_source=%r — falling back to 'legacy'.",
                _prompt_source,
            )
            _prompt_source = "legacy"
        self._memory_prompt_source = _prompt_source
        self._memory_suppress_builtin_when_external = bool(
                ...
                if _mem_provider_name:
                    ...
                    if self._memory_manager.providers:
                        _init_kwargs = {
                            ...
                            "prompt_source": getattr(self, "_memory_prompt_source", "legacy"),
    ...
    def _build_system_prompt_parts(self, system_message: str = None) -> Dict[str, str]:
        ...
        # Core prompt-source policy: decide whether the legacy MEMORY/USER
        # markdown blocks should be suppressed before we evaluate them.
        # The decision is provider-agnostic — we only inspect the rendered
        # external block. Provider-specific generation (ChromaDB, cache,
        # debug receipt) lives in the plugin lane.
        _prompt_source = getattr(self, "_memory_prompt_source", "legacy")
        _suppress_builtin_flag = getattr(
            self, "_memory_suppress_builtin_when_external", False
        )

        _external_block = ""
        _external_block_present = False
        _external_block_degraded = False
        if self._memory_manager:
            try:
                _external_block = self._memory_manager.external_system_prompt_block()
            except Exception as _ext_err:
                logger.debug(
                    "external_system_prompt_block() raised: %s — falling back to legacy",
                    _ext_err,
                )
                _external_block = ""
            _external_block_present = _provider_block_has_generated_profile(
                _external_block
            )
            _external_block_degraded = _provider_block_is_degraded(_external_block)

        # Whether to suppress the legacy memory_store MEMORY/USER blocks.
        # Suppression NEVER applies to the built-in status text (timestamp,
        # session/model lines) — those remain visible in every mode.
        # Degraded provider blocks are additive-only and never suppress
        # legacy (plan global rule 11).
        _suppress_legacy_memory = False
        if _prompt_source == "provider":
            # Strict provider mode:
            #   * non-degraded block present → provider replaces legacy.
            #   * block missing/empty       → no fallback; legacy hidden
            #                                   and visible-unavailable
            #                                   marker injected below.
            #   * degraded block            → additive only; legacy stays.
            if _external_block_degraded:
                _suppress_legacy_memory = False
            else:
                _suppress_legacy_memory = True
        elif _prompt_source == "provider_with_legacy_fallback":
            # Suppress legacy only when the provider block is present
            # and non-degraded. Missing or degraded → legacy stays.
            _suppress_legacy_memory = (
                _external_block_present and not _external_block_degraded
            )
        elif _prompt_source == "legacy":
            # Upstream #29020 semantics — only honored in legacy mode.
            _suppress_legacy_memory = (
                bool(_suppress_builtin_flag)
                and _external_block_present
                and not _external_block_degraded
            )
        # "shadow" mode never suppresses legacy.

        if self._memory_store and not _suppress_legacy_memory:
        ...
        # External memory provider system prompt block. In strict "provider"
        # mode this replaces legacy; in every other mode it is additive
        # (or fallback). When prompt_source == "provider" and the provider
        # produced nothing, inject the visible-unavailable marker so the
        # session is never silently profile-less.
        if self._memory_manager:
            if _prompt_source == "provider":
                if _external_block_present and not _external_block_degraded:
                    volatile_parts.append(_external_block)
                elif _external_block_degraded:
                    # Degraded provider output is additive-only in every mode:
                    # legacy stayed above, and the degraded block remains
                    # visible for operator/debug context.
                    volatile_parts.append(_external_block)
                else:
                    logger.warning(
                        "%s (prompt_source='provider', external block empty/degraded).",
                        _MEMORY_PROVIDER_UNAVAILABLE_MARKER,
                    )
                    volatile_parts.append(_MEMORY_PROVIDER_UNAVAILABLE_MARKER)
            else:
                # legacy / shadow / provider_with_legacy_fallback — external
                # provider output is appended at most once.  Team/status-only
                # blocks are still surfaced, but only <memory-profile> blocks
                # can suppress legacy.
                if _external_block and _external_block.strip():
                    volatile_parts.append(_external_block)
